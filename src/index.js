...
// Opposite-sides capture: an enemy cell flips when (N is mine AND S is mine)
// OR (E is mine AND W is mine). Heads count as their owner's color for blocking
// (a head on N is "mine" if I'm the head's owner — but heads are immune anyway).
//
// Chains: after each pass, any newly-flipped cells participate in finding new
// sandwiches. We loop until no more flips occur.
function captureFlanked(board, playerNum, size, heads) {
  const enemy = playerNum === 1 ? 2 : 1;
  const allFlipped = [];
  while (true) {
    const flippedThisPass = [];
    for (let y = 0; y < size; y++) {
      for (let x = 0; x < size; x++) {
        if (board[y][x] !== enemy) continue;
        if (isHeadAt(heads, x, y)) continue; // heads immune
        const at = (dx, dy) => {
          const nx = x + dx, ny = y + dy;
          if (!inBounds(nx, ny, size)) return null;
          // A head at this position counts as that head owner's color
          if (isHeadAt(heads, nx, ny)) {
            return heads.find(h => h.x === nx && h.y === ny).playerNum;
          }
          return board[ny][nx];
        };
        const ns = at(0, -1) === playerNum && at(0, 1) === playerNum;
        const ew = at(-1, 0) === playerNum && at(1, 0) === playerNum;
        if (ns || ew) {
          flippedThisPass.push([x, y]);
        }
      }
    }
    if (flippedThisPass.length === 0) break;
    for (const [x, y] of flippedThisPass) {
      board[y][x] = playerNum;
    }
    allFlipped.push(...flippedThisPass);
  }
  return allFlipped;
}
